...
- # 工作流学习
	- 工作流（如请假、报销等业务）是一整套业务流程，其中涉及到流程的多次流转，包括同意、驳回等操作，并且可能涉及多个岗位和人员的协作。每次发起业务时，都会创建一个**流程实例**，用于记录当前的状态信息，包括当前所在的节点、流程的历史轨迹等。在流程达到特定节点时，可以配置监听器来通知外部系统或触发特定操作。在用户任务节点上，需要用户或系统后台进行操作，以继续推动流程的流转。
	- BPMN：工作流规范