# 关于 [[Python]] Gemini SDK 的使用
	...
	- ```python
	  ...
	  def translate(source_lang: str, target_lang: str, source_text: str, *source_texts: List[str]):
	      ...
	      return result
	  ```
	- DOING **AI Agent（智能体）**，智能体能够根据预定义的目标，策略，环境去**自主地**做出决策和行动，并有能力在过程中调整和学习。Gemini允许用户提供一些函数定义作为上下文，然后作为智能体去响应用户的输入，生成对这些函数进行操作的命令或者干脆主动去执行这些函数。下面是一个咖啡订单系统，允许用户通过自然语言去下订单，询问菜单等操作。
	  :LOGBOOK:
