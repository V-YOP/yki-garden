# #snippet/shell 使用 sed 使用正则进行文件内容替换
	...
	- 此外，sed增加了下面的命令：
		...
		- `r 待插入文件`：在匹配的行后插入另一个文件的内容（另起一行）
		  logseq.order-list-type:: number
		- `w 待写入文件`：将匹配行写入到另一个文件；w命令解析文件名的行为特别奇怪，考虑还是不要用它，可能弄出来一堆奇怪名字的文件，如命令`sed '/orange/{p; w orange.txt}' test.txt`会创建文件名为`orange.txt}`的文件，命令也会失败。
		  logseq.order-list-type:: number
		- `N`：把下一行也读进来，让后面的操作一并处理
		  logseq.order-list-type:: number
	- sed命令有如下参数：
