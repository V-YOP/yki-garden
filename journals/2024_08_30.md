# 关于 [[Python]] 的生成器（协程）的`yield`，以及`yield from`的心智模型
	...
	- 协程结束执行后，它return的值会作为StopIteration异常的key弄出来
	...
	- ```python
	  ...
	  for i in x:
	      yield i
	  ```
	- 但考虑到协程是可以喂给它值的，现在事情变得复杂了。`yield from`现在应当理解为，**委派（delegate）**工作给**子生成器**，这个委派和其他语言的委派概念一样——实际工作不是我干的，**我只接受结果**，操作我就是操作我委派的那家伙。
