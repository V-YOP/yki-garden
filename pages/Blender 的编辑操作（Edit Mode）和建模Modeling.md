...
- ## 归并顶点 Merge Vertices
	- 在进行了许多复杂操作后，**模型中很可能存在重复的顶点，边和面**，这会让某些修改器Modifier产生问题。要处理掉重复的顶点，需要使用所谓的Merge，Merge把两个或多个顶点合并成一个。（这只是一种场景，另一个场景是确实想把两个顶点合成一个）
	- 要进行归并，需要**选择待归并的顶点，然后`右键-Merge Vertices`**，这里能选择归并的