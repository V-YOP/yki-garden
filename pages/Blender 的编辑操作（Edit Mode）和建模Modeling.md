...
- `Ctrl-V/E/F`能看到点，线，面的菜单。
- ## 对象原点origin
	- 对象有一个原点**作为其移动，旋转和缩放的根据**，对象的位置也是按原点位置去标识的，正方体的原点默认在体的正中心。**可以通过3D Cursor设置对象原点**，只需右键对象然后`Set Origin-Origin to 3D Cursor`。**此外，也可以通过应用位置的变换来设置对象原点**，这适用于物体正在世界原点附近进行编辑时。
...
- # 选择工具
- `B`进入框选模式，框选一个范围后会**增加**选中相应元素然后退出框选模式，这里按住Shift则是减少。也可以按鼠标中键去进行框选，此时也是移除
- `C`进入刷选模式，刷选模式需要按`鼠标右键`或`Esc`主动退出。按下Shift时再刷选会移除选择，或者按下鼠标中键。
- `L`选择和当前选择元素相连的点。
- `Alt-鼠标左键`会**循环选择**，`Ctrl-Alt-鼠标左键`会**在垂直方向上循环选择**。
- # 建模Modeling
...
- ## 环切Loop Cut
	...
	- `Ctrl-R`进入Loop Cut模式，此时：
		...
		- `Esc`以取消移动，做均等切分，否则应用移动
		  logseq.order-list-type:: number
- ## 倒角 Bevel
	- `Ctrl-B`能对边进行倒角。鼠标移动调整倒角大小，滚轮调整倒角数量。
- ## 连接 Connect
	- 选择一些点，然后按`F`，就能连接它们。**两个点创建线，更多点创建面**。
	- 一般而言，要避免三角形的面，它会破坏边环，变形动画时也会不好看。
	- 此外，还有`Bridge Faces`——选择两个面，然后`右键-Bridge Faces`，这能够**把两个面通过面完全连接起来**，这个连接很聪明，在发现连接后，面处在“内部”时把它移除掉。**Bridge Faces同时可以用来开孔**。此外，还有`Bridge Edge Loops`，字面意思，桥接循环边。
	- F不会自动分割面，有时候`F`不起效。在**尝试切割面的时候，考虑用`J`，它在连接的时候会把面进行split**。尝试无中生有的时候，使用`F`。
	- `F`会创建有许多边的面，如果希望面能被三角形或四边形填充，使用fill功能`Alt-F`，或者`Ctrl-F-Grid Fill`。
- ## 分离Separate和合并Join
	- 急流岩上碎，无奈两离分
...
- ## 断开
	- `V`能断开一个点或线，断开即把它拷贝一份，然后让和它相邻的某个线或面转而去连接拷贝的那份。
- ## 归并顶点 Merge Vertices
...
- ## 切刀Knife
- `K`，顾名思义，切刀。**回车键或空格退出，右键从当前刀上退出，Esc撤销**。切刀是从视图出发的，所以一次可以从多个面上切。
- ## 镜像Mirror
